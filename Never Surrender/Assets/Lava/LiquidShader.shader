        ...
        SubShader
        {
            ...
            CGPROGRAM
                ...
                fixed4 drawWater(fixed4 water_color, sampler2D color, float transparency, float height, float angle, float wave_strength, float wave_frequency, fixed2 uv)
                {
                    ...
                    wave += sin(30.0 * -uv.y + 30.0 * uv.x + wave_frequency * 1.0 * iTime) * wave_strength * 0.5;
                    wave += sin(20.0 * -uv.y + 20.0 * -uv.x + wave_frequency * 0.6 * iTime) * wave_strength * 1.3;
                    wave += sin(4.0 * -uv.y + 4.0 * -uv.x + wave_frequency * 0.3 * iTime) * wave_strength * 10.0;
